...
public:
   ...
   */
  int ColCalc(int row, int leftCol, int rightCol) const;
  int RowCalc(int rowTop, int rowBottom, int col) const;
  int** FillingEnergyMatrix() const;
  int** InitiliazeMatriz() const;
  int** PopulatingVerticalMatrix() const;
  int** PopulatingHorizontalMatrix() const;

  void ClearMatrix(int**& arr) const;
  void ClearOneDArray(int*& arr);

  void PrintMatrix(int** matrix) const;
  void HelperForHorizontalSeam(int**& horizontal_seam_matrix,
                               int& starting_index,
                               int*& best_optimal_indexes) const;
  void HelperForVerticalSeam(int**& vertical_seam_matrix,
                             int& starting_index,
                             int*& best_optimal_indexes) const;
...
private:
  ...
  int height_ = 0;
  int width_ = 0;
  // int** vertical_seam_matrix_ = nullptr;
  /**
   * Add any helper methods you may need
   */
  /*int ColCalc(int row, int leftCol, int rightCol) const;
